...
function getUserAgent() {
  return isEmpty(utils.userPlatform)
    ? `${utils.USER_AGENT}`
...
function textStyle(layer) {
  ...
  if (
    layer.hasOwnProperty('font_size' || 'font_family') ||
    !isEmpty(keywords)
  ) {
    if (!layer.font_size)
...
function normalize_expression(expression) {
  ...
  expression = expression.replace(
    operatorsReplaceRE,
    (match) => CONDITIONAL_OPERATORS[match]
  )

  const predefinedVarsPattern =
    '(' + Object.keys(PREDEFINED_VARS).join('|') + ')'
...
function process_custom_function(customFunction) {
  ...
  if (customFunction.function_type === 'remote') {
    return [
      ...
      URLSafeBase64.encode(base64EncodeURL(customFunction.source)),
    ].join(':')
  }
...
function build_upload_params(options) {
  let params = {
    ...
    allowed_formats:
      options.allowed_formats && toArray(options.allowed_formats).join(','),
    ...
    discard_original_filename: utils.as_safe_bool(
      options.discard_original_filename
    ),
    ...
    responsive_breakpoints: utils.generate_responsive_breakpoints_string(
      options.responsive_breakpoints
    ),
...
function build_eager(transformations) {
  return toArray(transformations)
    .map((transformation) => {
      const transformationString = utils.generate_transformation_string(
        clone(transformation)
      )
      const format = transformation.format
      return format == null
        ? transformationString
        : `${transformationString}/${format}`
    })
    .join('|')
}
/**
...
function generate_transformation_string(options) {
  ...
  if (isArray(options)) {
    return options
      .map((t) => utils.generate_transformation_string(clone(t)))
      .filter(utils.present)
      .join('/')
  }

  let responsive_width = consumeOption(
    options,
    'responsive_width',
    config().responsive_width
  )
  ...
  let base_transformations = toArray(
    consumeOption(options, 'transformation', [])
  )
  let named_transformation = []
  if (base_transformations.some(isObject)) {
    base_transformations = base_transformations.map((tr) =>
  ...
  if (isObject(border)) {
    border = `${
      border.width != null ? border.width : 2
    }px_solid_${(border.color != null ? border.color : 'black').replace(
      /^#/,
      'rgb:'
    )}`
  ...
  if (options.offset != null) {
    ;[options.start_offset, options.end_offset] = split_range(
      consumeOption(options, 'offset')
    )
  }
  ...
  let custom_function = process_custom_function(
    consumeOption(options, 'custom_function')
  )
  let custom_pre_function = process_custom_pre_function(
    consumeOption(options, 'custom_pre_function')
  )
  ...
  let variables = entries(options)
    ...
    .concat(
      variablesParam.map(
        ([name, value]) => `${name}_${normalize_expression(value)}`
      )
    )
    .join(',')
  ...
  if (responsive_width) {
    let responsive_width_transformation =
      config().responsive_width_transformation ||
      DEFAULT_RESPONSIVE_WIDTH_TRANSFORMATION

    transformations.push(
      utils.generate_transformation_string(
        clone(responsive_width_transformation)
      )
    )
  }
...
function url(public_id, options = {}) {
  ...
  let force_version = consumeOption(
    options,
    'force_version',
  ...
  let long_url_signature = !!consumeOption(
    options,
    'long_url_signature',
    config().long_url_signature
  )
  ...
  let cdn_subdomain = consumeOption(
    options,
    'cdn_subdomain',
    config().cdn_subdomain
  )
  let secure_cdn_subdomain = consumeOption(
    options,
    'secure_cdn_subdomain',
  ...
  let use_root_path = consumeOption(
    options,
    'use_root_path',
    config().use_root_path
  )
  ...
  if (sign_url && isEmpty(auth_token)) {
    ...
    signature = shasum
      ...
      .replace(/\//g, '_')
      .replace(/\+/g, '-')
      .substring(0, long_url_signature ? 32 : 8)
...
function finalize_source(source, format, url_suffix) {
  ...
  if (source.match(/^https?:\//i)) {
    ...
    source = encodeURIComponent(decodeURIComponent(source))
      .replace(/%3A/g, ':')
      .replace(/%2F/g, '/')
    source_to_sign = source
    if (url_suffix) {
...
function finalize_resource_type(
  resource_type,
  type,
  url_suffix,
  use_root_path,
  ...
  if (use_root_path) {
    if (
      ...
      (resource_type === 'images' && type == null)
    ) {
      resource_type = null
      type = null
    } else {
...
function unsigned_url_prefix(
  ...
  if (secure) {
    if (
      ...
      secure_distribution === exports.OLD_AKAMAI_SHARED_CDN
    ) {
    ...
    if (secure_cdn_subdomain) {
      secure_distribution = secure_distribution.replace(
        'res.cloudinary.com',
...
function download_archive_url(options = {}) {
  let cloudinary_params = exports.sign_request(
    exports.archive_params(
      merge(options, {
        mode: 'download',
      })
    ),
  ...
  return (
    exports.api_url('generate_archive', options) +
    '?' +
    hashToQuery(cloudinary_params)
  )
}
...
function download_zip_url(options = {}) {
  return exports.download_archive_url(
    merge(options, {
      target_format: 'zip',
    })
  )
...
function cloudinary_js_config() {
  ...
  return `<script type='text/javascript'>\n$.cloudinary.config(${JSON.stringify(
    params
  )});\n</script>`
}
...
const OFFSET_ANY_PATTERN_RE = RegExp(
  `(${OFFSET_ANY_PATTERN})\\.\\.(${OFFSET_ANY_PATTERN})`
)
...
function archive_params(options = {}) {
  return {
    ...
    flatten_transformations: exports.as_safe_bool(
      options.flatten_transformations
    ),
    ...
    fully_qualified_public_ids:
      options.fully_qualified_public_ids &&
      toArray(options.fully_qualified_public_ids),
    public_ids: options.public_ids && toArray(options.public_ids),
    skip_transformation_name: exports.as_safe_bool(
      options.skip_transformation_name
    ),
...
exports.create_source_tag = function create_source_tag(
  src,
  source_type,
  codecs = null
) {
...
function build_streaming_profiles_param(options = {}) {
  let params = pickOnlyExistingValues(
    options,
    'display_name',
    'representations'
  )
  if (isArray(params.representations)) {
    params.representations = JSON.stringify(
      params.representations.map((r) => ({
